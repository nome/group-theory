...
{-
 ...
 -}
knuthBendixRewrite :: (Eq a, Ord a) => [a] -> [Word a] -> RewriteRules a
knuthBendixRewrite generators relators = extend baseRules
	where
		...
		reduce rules = if selfApply == rules then rules else reduce selfApply
			where selfApply = do
				...
				if null wl then [] else return (left', Word[])
		...
		findConfluenceExceptions rules = do
			...
			if p == p'|| null overlap || r == r' then [] else return newRule
...
data PresentedGroup a = PresentedGroup (RewriteRules a) (Word a)

-- constructor function for convenience
presentedGroup :: (Eq a, Ord a) => [a] -> [Word a] -> Word a -> PresentedGroup a
presentedGroup generators relators element = PresentedGroup (knuthBendixRewrite generators relators) element

instance Eq a => Group (PresentedGroup a) where
	(PresentedGroup rules (Word w)) ° (PresentedGroup rules' (Word w'))
		| null w = PresentedGroup rules' (Word w')
		| null w' = PresentedGroup rules (Word w)
		| rules /= rules' = undefined
		| otherwise = let Word w'' = applyRewriteRules rules (Word (w++w')) in PresentedGroup (if null w'' then [] else rules) (Word w'')
	neutral =  PresentedGroup [] (Word [])
	inverse (PresentedGroup rules (Word w)) = PresentedGroup rules $ applyRewriteRules rules $ Word $ reverse $ map invert w
		where
			invert (Left x) = (Right x)
			invert (Right x) = (Left x)

instance Show a => Show (PresentedGroup a) where
	show (PresentedGroup rules word) = show word

instance Eq a => Eq (PresentedGroup a) where
	(PresentedGroup rules word) == (PresentedGroup rules' word') = setEqual rules rules' && word == word'

instance Ord a => Ord (PresentedGroup a) where
	compare (PresentedGroup _ word) (PresentedGroup _ word') = compare word word'
