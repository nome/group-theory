...
{-
 - The free group generated by values of a type.
 -}
...
{-
 ...
 -
 - Note that in a small nod towards a halfway usable runtime efficiency, words are rewritten to
 - canonical form (using the subjugate function) whenever a presentation coset is created and not
 ...
 -}
data PresentationCoset a = PresentationCoset [Either a a]

{-
 - Canonical map from the free group to its quotient group defined by a presentation.
 - Use the relations of the presentation to translate a given word in the generators to a canonical
 - form. Uses Knuth-Bendix completion algorithm for generating the translation rules.
 -}
subjugate :: Presentation a => Word a -> PresentationCoset a
subjugate w = let Word reduced = applyRewriteRules canonicalizationRules w in PresentationCoset reduced
...
canonicalizationRules = knuthBendixRewrite (enumFromTo minBound maxBound) relators
...
instance Eq a => Eq (PresentationCoset a) where
	(PresentationCoset w) == (PresentationCoset w') = (Word w) == (Word w')
